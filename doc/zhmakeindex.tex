...
"\indexentry" 是在索引输入文件中表示索引项的关键字。可以使用 "keyword" 输入格
式进行修改（\autoref{tab:oldinputstyle}）。左右花括号是界定 \meta{条目} 与
\meta{页码} 的标记，可以使用 "arg_open", "arg_close" 输入格式修改
...
\sitem X 射线, 9
\end{idxexample}
即分隔符 "!" 的优先级低于 "@"。分隔符 "!" 可通过修改格式文件的 "level" 项配置
（\autoref{tab:oldinputstyle}）。

索引项条目中也可以在最后指定页码的输出格式，用分隔符 "|" 与前面分开（用
\autoref{tab:oldinputstyle} 中的 "encap" 项配置）。页码的输出格式由一个带单个
页码参数的 \TeX{} 命令决定，在输入索引项时这一特殊命令的反斜线 "\" 和左右花括
号被忽略，输出时会自动加上（用\autoref{tab:oldoutputstyle} 中的
"encap_prefix", "encap_infix", "encap_suffix" 项配置）。例如：
\begin{idxexample}
...
\end{idxexample}
页码范围的开闭定界符可由\autoref{tab:oldoutputstyle} 中的 "range_open" 与
"range_close" 项配置。
...
绝对值 $\vert x\vert$, 2
\end{idxexample}
引号字符 |"| 可以由\autoref{tab:oldinputstyle} 中的 "quote" 项配置。
...
命令 \verb+\|+, 9
\end{idxexample}
转义符 "\" 可以由\autoref{tab:oldinputstyle} 中的 "escape" 项配置。
